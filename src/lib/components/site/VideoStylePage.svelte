<script>
  ...
  // Lifestyle Video / Campaign). One component, driven by the `style` data
  // object from $lib/data/videoStyles.js — see
  // src/routes/video/[slug]/+page.svelte for how it's wired up. Mirrors
  // CatalogStylePage.svelte / StylePage.svelte's structure; Campaign's CTA
  // is a WhatsApp quote request rather than an internal order link, so the
  // buttons are driven by style.ctaHref/ctaLabel/ctaExternal instead of a
  // hardcoded order route.
...
<section class="page-hero">
  <div class="container">
    <p class="eyebrow-page"><a href="/video" style="color:inherit">Video styles</a></p>
...
<!-- Media band — plain fade-reveal on a cine-format ProductScene, matching
     the source pages' icon-only .vis.cine (no real photo). -->
<section class="section-tight">
  <div class="container">
    <div class="reveal pending" use:reveal>
      <ProductScene icon={style.heroIcon} width={style.heroWidth} badge="Video &middot; {style.name}" cine />
    </div>
  </div>
</section>
  ...
  <div class="container">
    <div class="grid grid-3">
      {#each style.grid as g}
        <div class="reveal pending" use:reveal>
          <ProductScene photo={g.photo} icon={g.icon} width={g.width} badge="Video &middot; {style.name}" />
        </div>
      {/each}
    </div>
  </div>
